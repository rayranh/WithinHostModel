...
creating_plots <- function(listofdf, i) { 
  ...
  df_for_40000 <- df %>% mutate(cytolytic_scale_40000 = ((Cb+Ct)/(B_cells+Cb+Ct+T_cells+At+Br+Lt+Lt2+Lt3+Lt4+Lt5))* 120000, 
                                cytolytic_scale_40000_cb = ((Cb)/(B_cells+Cb+Ct+T_cells+At+Br+Lt+Lt2+Lt3+Lt4+Lt5))* 120000, 
                                cytolytic_scale_40000_ct = ((Ct)/(B_cells+Cb+Ct+T_cells+At+Br+Lt+Lt2+Lt3+Lt4+Lt5))* 120000) 
  ...
  # PBL_plot <- ggplot(data = df_PBL, aes(x = time/24, y = PBL_scale_10000)) + geom_line( color = "red")  +
  #   labs(title = "WithinHost Delay", color = "Cell Type")  + xlab(label = "Time (Days)") +
  #   ylab(label = "Cell Number")  +  scale_y_log10(limits = c(0.0000001, 1000000000000)) +
  #   theme(panel.grid = element_blank(), panel.background = element_blank(),   legend.text = element_text(size = 12),
  #         legend.title = element_text(size = 12), axis.line = element_line(color = "black")) +
  #   labs( y = "Cell Number", x = "Time (days post infection)", title = "Infected PBL") + 
  #   theme_classic()
  # 
  # 
...
optim_data <- read.csv("/Users/rayanhg/Desktop/WithinHostModel/CodeOutputsRandNum/Fit_MDV_CombinedPartial_v01.csv") %>% 
  filter(Converged == 0) %>% slice_min(Likely, n = 10) %>% arrange(Likely) %>% select(c(beta, alpha, alpha_2,nu_a,nu_f,mu,Pb, size_pp38, size_pp38_Ct, g1,g2, h1,h2, beta_2,nu_b)) 
...
pdf("/Users/rayanhg/Desktop/WithinHostModel/CodeOutputsRandNum/Fit_MDV_CombinedPartial_v01.pdf", width = 7, height = 5)
