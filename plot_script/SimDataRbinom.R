...
baigent1998 <- read_xlsx("Baigent1998/baigent1998.xlsx", 3 ) %>% 
  mutate(mean.pp38 = as.numeric(mean.pp38),
         Bcell_no = as.numeric(Bcell_no), 
         Tcell_no = as.numeric(Tcell_no)) %>% filter(!is.na(mean.pp38))   
optim_data <- read.csv("/Users/rayanhg/Desktop/WithinHostModel/CodeOutputsRandNum/Jun.4.26.FittingMDVModelClusterCombined.csv") %>% 
  filter(Converged == 0) %>% slice_min(Likely,n = 10) %>%  
  select(c(beta,  alpha, alpha_2,nu_a,nu_f,mu,g1,g2,h1,h2,Pb, size_pp38, size_pp38_Ct)) 
...
p_cb <- ggplot(sim_df_median, aes(x = time/24)) + geom_line(aes(y = mean_Cb, colour = "model B cells")) +  
  ...
  theme_classic() # im plotting 0's  

p_data <- ggplot(baigent1998, aes(x = time/24, y = Tcell_no, color = "data")) + 
  geom_point() + xlim(0,10) + scale_color_manual(values = c("model" = "black", "data" = "red")) +  
  labs(title = "T cell infection Data", x = "time(days post infection)", y = "# of Infected Cells per 40,000 lymphocytes") + ylim(0,80) +
  theme(panel.grid = element_blank()) + 
  theme_classic()

p_Ct <- ggplot(sim_df_median, aes(x = time/24)) + geom_line(aes(y = mean_Ct, colour = "model T cells")) +  
...
p2 <- ggplot(sim_FFE_median, aes(x = time/24))  + geom_line(aes(y = med, colour = "model")) + 
  ...
  geom_errorbar(data = baigent2016, aes(ymin = mean_genomes, ymax = ConfInt, color = "data")) + 
