...
Likelihood <- function(params){   
  ...
  # pars["M"] <-  abs(params[5])
  # pars["nu_A"] <-  abs(params[6])
  # pars["nu_b"] <-  abs(params[7]) 
  # pars["alpha"] <-  abs(params[6])
  # pars["alpha_2"] <-  abs(params[7]) 
  # pars["alpha_B"] <-  abs(params[8])
  # pars["theta"] <-  abs(params[10]) 
  # pars["g1"] <-  abs(params[11])
  # pars["g2"] <-  abs(params[12]) 
  # pars["h1"] <-  abs(params[13]) 
  # pars["h2"] <-  abs(params[14]) 
  # pars["lambda"] <-  abs(params[15]) 
  # pars["epsilon"] <-  abs(params[16])
  # 
  ...
  ####### GRAPHS TO SEE WHATSAGOOINGON ######### 
  
  # long_df_bu <- long_results %>% filter(variable %in% c("B_bu", "T_bu", "Ct_bu", "Cb_bu", "Lt_bu", "Z_bu", "At_bu")) 
  # long_df_sp <- long_results %>% filter(variable %in% c("B_cells", "T_cells", "Ct", "Cb", "Lt", "Z_sp", "At")) 
  ...
  dtotal <-  results %>% mutate(B_total = rowSums(across(c(Bb_cells, Cbb_cells))/76000)) %>% mutate(T_total = rowSums(across(c(Tb_cells, Atb_cells, Ltb_cells, Ctb_cells))/76000)) %>% 
  ...
  infprob <- results %>% select(B_cells, Cb, Ct, T_cells, At,Lt, Lt2, Lt3, Lt4, Lt5, time)  %>% mutate(prob_Cyto = (Cb+Ct)/(B_cells+Cb+Ct+T_cells+At+Lt+Lt2+Lt3+Lt4+Lt5))
  infprob_Bu <- results %>% select(T_bu, B_bu, Cb_bu, Ct_bu, At_bu,Lt_bu, Lt2_bu, Lt3_bu, Lt4_bu, Lt5_bu, time)  %>%  mutate(prob_Cyto_bu = (Cb_bu + Ct_bu)/(T_bu+ B_bu+Cb_bu+Ct_bu+At_bu+Lt_bu+Lt2_bu+Lt3_bu+Lt4_bu+Lt5_bu)) 
  infprob_Th <- results %>% select(T_th, B_th, Cb_th, Ct_th, At_th,Lt_th, Lt2_th, Lt3_th, Lt4_th, Lt5_th, time)  %>%  mutate(prob_Cyto_th = (Cb_th + Ct_th)/(T_th+B_th+Cb_th+Ct_th+At_th+Lt_th+Lt2_th+Lt3_th+Lt4_th+Lt5_th)) 
  ...
  for (i in seq_len(nrow(matchedTime))) { 
    obs_cytoBcells <- pp38_Spleen %>% filter(time %in% matchedTime$time[i]) %>% pull(mean.pp38) # matching data to model time for B  
    loglike_B_pp38 <- dbinom(obs_cytoBcells, 40000, prob = matchedTime$prob_Cyto[i], log = TRUE) #binomial doesnt like when = 1?    
    LogLHoodStor=LogLHoodStor+sum(loglike_B_pp38,na.rm= TRUE)
    #result[[i]] <- data.frame(time = rep(dtotallike$time[i], length(obs_vec)) , obs = obs_vec, model_val = rep( mu, length(obs_vec)) , likelihood = loglike) 
    
  }  
  for (i in seq_len(nrow(matchedTime))) { 
    obs_cytoBcells_Bu <- pp38_Bursa %>% filter(time %in% matchedTime$time[i]) %>% pull(mean.pp38) # matching data to model time for B  
    loglike_B_pp38_Bu <- dbinom(obs_cytoBcells_Bu, 40000, prob = matchedTime_Bu$prob_Cyto_bu[i], log = TRUE) #binomial doesnt like when = 1?    
    LogLHoodStor=LogLHoodStor+sum(loglike_B_pp38_Bu,na.rm= TRUE)
    #result[[i]] <- data.frame(time = rep(dtotallike$time[i], length(obs_vec)) , obs = obs_vec, model_val = rep( mu, length(obs_vec)) , likelihood = loglike) 
    
  }  
  for (i in seq_len(nrow(matchedTime))) { 
    obs_cytoBcells_Th <- pp38_Thymus %>% filter(time %in% matchedTime$time[i]) %>% pull(mean.pp38) # matching data to model time for B  
    loglike_B_pp38_Th <- dbinom(obs_cytoBcells_Th, 40000, prob = matchedTime_Th$prob_Cyto[i], log = TRUE) #binomial doesnt like when = 1?    
    LogLHoodStor=LogLHoodStor+sum(loglike_B_pp38_Th,na.rm= TRUE)
    #result[[i]] <- data.frame(time = rep(dtotallike$time[i], length(obs_vec)) , obs = obs_vec, model_val = rep( mu, length(obs_vec)) , likelihood = loglike) 
...
parameters_values <- c(
  M = 1.038840e-3
  , beta =2.185018e-06   #contact rate with B cells
  , beta_2 = 3.238362e-02  #contact rate with T cells
...
test_parms <- c( 1.185018e-10, 1.238362e-20)
