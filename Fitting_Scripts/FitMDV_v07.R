...
Likelihood_parts <- function(params){   
  ...
  infprob <- results %>% 
    ...
    mutate(
      ...
      inf_cells_Ct = ((Ct) / (B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5))*40000
...
parameter_intervals <- list( beta_2 = log(c(1e-24,1e-18)), 
                             ...
                             size_pp38_Ct = log(c(0.05,0.5)))  #taken from baigent pp38 data 
...
out_file <- sprintf("~/work/FitMDV_v07_RandomStartDifferences%03d.csv", task) #outputting separate files 
