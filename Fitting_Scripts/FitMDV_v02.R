...
Likelihood_parts <- function(params){   
  ...
  Infected_FFE<- results %>% dplyr::select("time", "If", "f") %>% 
    filter(time %in% matched_time) %>% arrange(time) %>% mutate(IfPer10K = (If / (If + f)) * 10000)  
...
parameter_intervals <- list( beta_2 = log(c(1e-14,1e-5)), 
                             beta = log(c(1e-14, 1e-5)), 
...
out_file <- sprintf("~/work/FitMDV_v02_%03d.csv", task) #outputting separate files 
