...
Likelihood_parts <- function(params){   
  ...
  infprob <- results %>% 
    dplyr::select(B_cells, Cb,Br, Ct, T_cells, At, Lt, Lt2, Lt3, Lt4, Lt5,Lr, time) %>% 
    mutate(
      prob_Cyto = (Cb + Ct + Lr) / (B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr),
      inf_cells = ((Cb + Ct + Lr) / (B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr))*40000,
      inf_cells_Cb = ((Cb) / (B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr))*40000,  
      inf_cells_Ct = ((Ct + Lr) / (B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr))*40000
  ...
  InfectedPBL <- results %>% mutate(InfCellPBL = (Cb + Ct + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr)/
                                      (B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr) * 10000) %>% 
...
### Scaling Plaque assay Data ### 
  PlaqueCells <- results %>% mutate(pfuCells = (Cb + Ct + Lr)/(B_cells + Cb + Ct + T_cells + At + Br + Lt + Lt2 + Lt3 + Lt4 + Lt5 + Lr) * 1000000) %>% 
...
out_file <- sprintf("~/work/FitMDV_v09_AddedReactivationAddedPFU%03d.csv", task) #outputting separate files 
