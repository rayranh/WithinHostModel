#v05- Changed to lognormal distribution, Getting rid of Recruitment, make beta & beta_2 the same, make nu_a and nu_b the same 
...
Likelihood_parts <- function(params){   
  ...
  loglike_ffe <- dnorm(
    ...
    sd =  0.4971824, # SE calculated from 17dpi onwards, calculated this way because no n for chicken feathers only PBL 
  ...
  loglike_PBL2016 <- dnorm(
    ...
    sd =  0.3542103, # sqrt(mean(SElog^2)), SE calculated by n chicks 
    log = TRUE # outer log is TRUE so it is returning ln() to keep consistent
  )  
...
obs_hourspp38 <- c(
  0, 48, 72, 96, 120, 144,
  192, 360, 720, 1200
)# make sure pp38 times is matching the data loaded in 
...
pp38_dat <-  read_xlsx("~/work/baigent1998.xlsx", sheet = 4, na = "NA") %>% filter(!is.na(mean.pp38)) %>% select(time, Bcell_no,Tcell_no)
...
cortes2004_plaque <- read_xlsx("~/work/RB1B_plaqueAssayPchicks.xlsx")
...
out_file <- sprintf("~/work/SimplifyingModel_v05_added0_%03d.csv", task) #outputting separate files 
