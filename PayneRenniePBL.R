...
p2 <- ggplot(data = PayneLongT) +geom_point(mapping =aes(x = days, y= value,group = variable, colour = variable), position = position_jitter(width = 0.55, height =0), size = 1.5) + 
  scale_y_log10(limits = c(1000, 100000), breaks = c(1000, 10000, 100000), minor_breaks = c(2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000 ,20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000)) + 
  scale_x_continuous(limits = c(0,45), breaks = seq(0,45,by=5)) + geom_line(data = PayneMeanLongDfT, mapping = aes(x = days, y = value, group = variable, colour = variable, linetype = variable)) + annotation_logticks(base = 10, sides = "l") +
  scale_linetype_manual(values = c("antilogT" = "dashed", "antilogCtrlT"= "solid")) + scale_color_manual(values = c("antilogT"="indianred2", "antilogCtrlT"="mediumseagreen", "InfectTcells"= "indianred2", "TCellsControl"= "mediumseagreen"))+ theme()
