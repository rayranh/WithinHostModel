...
ggplot(data = PayneLongB) +geom_point(mapping =aes(x = days, y= value,group = variable, colour = variable), position = position_jitter(width = 0), size = 2) + 
  ...
  scale_linetype_manual(values = c("antilog" = "dashed", "antilogCtrl"= "solid")) + theme()
